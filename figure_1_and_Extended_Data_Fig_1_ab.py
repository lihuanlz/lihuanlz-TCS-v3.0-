...
# =============================================================================
def _view(y, lo, hi):
    """Mask curve values outside the visible y-range so that NO line geometry
    exists beyond the axes. This makes the exported SVG robust against
    converters that drop SVG clip-paths (e.g. submission-system PDF mergers)."""
    return np.ma.masked_where((np.asarray(y, dtype=float) < lo) |
                              (np.asarray(y, dtype=float) > hi), np.asarray(y, dtype=float))
...
def figure3_quantification_performance(kappa=1, beta=1, N=1e6, n=1e4, b=0.01, panel='b'):
    """
    ...
    ax2.semilogx(M_test, _view(CV_R1 * 100, -50, 200), 'k-', lw=3, label='R1 CV (%)')
    ax2.semilogx(M_test, _view(CV_R2 * 100, -50, 200), 'b--', lw=2, label='R2 CV (%)')
    ax2.semilogx(M_test, _view(CV_R3 * 100, -50, 200), 'r--', lw=2, label='R3 CV (%)')
    ax2.semilogx(M_test, _view(bias_R2, -50, 200), 'b:', lw=2, alpha=0.6, label='R2 Bias (%)')
    ax2.semilogx(M_test, _view(bias_R3, -50, 200), 'r:', lw=2, alpha=0.6, label='R3 Bias (%)')
    ...
    for ni, color in zip(n_values, n_colors):
        ...
        ax3.semilogx(M_test, _view(CV_temp * 100, 0, 200), color=color, lw=2.5, label=label)
    ...
    for bi, color in zip(b_values, b_colors):
        ...
        label = f'R1, b={bi}'
        ax4.semilogx(M_test, _view(CV_temp * 100, 0, 200), color=color, lw=2.5, label=label)
    ...
    for n_b, color in zip(n_blank_list, n_blank_colors):
        ...
        ax5.semilogx(b_range, _view(RSE_exact, 0, 150), color=color, lw=2.5, label=f'n={n_b:.0e}')
    ...
    plt.suptitle(f'Extended Data Fig. 1{panel}: Model Validation — Quantification Performance and Parameter Sensitivity.', 
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = os.path.join(output_dir, f"Extended_Data_Fig_1{panel}.svg")
    plt.savefig(filename, format='svg', bbox_inches='tight')
