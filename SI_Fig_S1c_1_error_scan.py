...
from tqdm import tqdm          # progress bar; if missing: pip install tqdm
import matplotlib
...
# ----- Parameter grid -----
Neff_vals = np.arange(3, 201)            # 198 points
kappa_vals = np.logspace(-2, 2, 50)      # 50 points

# Initialize result matrices
ERR = np.full((len(kappa_vals), len(Neff_vals)), np.nan)

# Double loop + progress bar
...
plt.tight_layout()
# Save first, then show (the Spyder plot window opens automatically)
plt.savefig('S1c.1.svg', dpi=300, bbox_inches='tight')
plt.show()          # usually appears directly in Spyder; optional
